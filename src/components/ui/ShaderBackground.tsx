...
// Recolored to MediVault's teal/obsidian palette (#2DD4A8 accent on #08070A base)
...
void main(void) {
  ...
  for (float i=1.; i<12.; i++) {
    ...
    vec2 p=uv;
    float d=length(p);
    col+=.00125/d*(cos(sin(i)*vec3(.3,.9,1.4))*vec3(.18,.83,.66)+vec3(.18,.83,.66));
    float b=noise(i+p+bg*1.731);
    col+=.002*b/length(max(p,vec2(b*p.x*.02,p.y)))*vec3(.18,.83,.66);
    col=mix(col,vec3(bg*.045,bg*.21,bg*.165),d);
  }
  O=vec4(col,1);
}`;
